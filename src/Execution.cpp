...
Execution::Execution(const std::string &filename) { this->loadFile(filename); }

std::string Execution::getValue() { return this->_value; }

void Execution::setValue(const std::string &value) { this->_value = value; }
...
void Execution::display() {
  ...
  for (auto const &key : getKeys(this->_inputs)) {
    std::string state =
        this->_inputs[key]->getState() == nts::Tristate::UNDEFINED
            ? "U"
            : std::to_string(this->_inputs[key]->getState());
    std::cout << "  " << key << ": " << state << std::endl;
  }
  std::cout << "output(s):" << std::endl;
  for (auto const &key : getKeys(this->_outputs)) {
    std::string state =
        this->_outputs[key]->getState() == nts::Tristate::UNDEFINED
            ? "U"
            : std::to_string(this->_outputs[key]->getState());
    std::cout << "  " << key << ": " << state << std::endl;
  }
}
...
void Execution::input(const std::string &value) {
  ...
  if (buffString.empty() || !this->_inputs.contains(buffString))
    throw nts::NanoTekSpiceError("Input \"" + buffString +
                                 "\" does not exist.");
  if (std::regex_search(ALL(value), match, b) == 1) {
    if (match[0] == "=1")
      ...
      throw nts::NanoTekSpiceError(
          "Left hand of assignment operator should be 1 or 0.");
  }
  ...
  try {
    ...
    while (std::getline(input_file, line)) {
      ...
      if (section == 2) {
        ...
        this->circuitry[compNameLeft]->setLink(
            pinNumLeft, *this->circuitry[compNameRight], pinNumRight);
      }
...
Execution::createComponent(const std::string &type) {
  return createComponent(type, type + std::to_string(random() % 100000));
}

std::unique_ptr<nts::IComponent>
Execution::createComponent(const std::string &type, const std::string &name) {
  ...
  if (type == "input") {
    ...
    this->_inputs[name] =
        reinterpret_cast<nts::InputComponent *>(new nts::ClockComponent(name));
    component = this->_inputs[name];
  } else if (type == "true")
