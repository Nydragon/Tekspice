...
nts::InvalidLink::InvalidLink(const std::string &type, const std::string &line)
    : NanoTekSpiceError("Right hand side of link for component \"" + type +
                        "\" has to refer to self, here: \"" + line + "\"") {}

nts::SyntaxError::SyntaxError(const std::string &line, int line_no)
    : NanoTekSpiceError("Line " + std::to_string(line_no) + ": \"" + line +
...
void nts::validateArgs(int count, char *args_c[]) {
  ...
  try {
    ...
    for (const auto &arg : args) {
      if (arg == "-h") {
        std::cout << "Enter a filename in param : ./nanotekspice [filename]"
                  << std::endl;
        exit(0);
      } else {
...
nts::PinNotFoundError::PinNotFoundError(const std::string &name, size_t pin)
    : NanoTekSpiceError("Pin \"" + std::to_string(pin) + "\" not found in " +
                        name + ".") {}
...
nts::ComponentNotFoundError::ComponentNotFoundError(const std::string &name,
                                                    int line_no)
    : NanoTekSpiceError("Line " + std::to_string(line_no) + ": Component \"" +
                        name + "\" does not exist.") {}
...
nts::RegexFailedError::RegexFailedError(int line_no)
    : NanoTekSpiceError("Line " + std::to_string(line_no) +
