...
#include "../LogicGates/NandGate.hpp"
#include "../LogicGates/NorGate.hpp"
#include "../LogicGates/NotGate.hpp"
#include "../LogicGates/OrGate.hpp"
#include "../LogicGates/SplitterGate.hpp"
#include "../LogicGates/XnorGate.hpp"
#include "../LogicGates/XorGate.hpp"
#include "../NanoTekSpiceError.hpp"
#include <cstring>
#include <fstream>
#include <regex>

nts::GenericComponent::GenericComponent(const std::string &type,
  ...
  while (std::getline(input_file, line)) {
      ...
      if (line == ".pins") {
        ...
        for (size_t i = 0; i < this->_pin_no; i++)
          this->pins[i] = {.number = i + 1,
                           .state = std::make_unique<nts::Tristate>(
    ...
    if (section == 2 && left != "null") {
      ...
      std::string compNameLeft =
          std::regex_token_iterator(C_ALL(left), beforeCol, 0)->str();
      std::string compNameRight =
          std::regex_token_iterator(C_ALL(right), beforeCol, 0)->str();
      /**
       ...
       */
      size_t pinNumLeft = std::stoi(
          &std::regex_token_iterator(C_ALL(left), afterCol, 0)->str()[1]);
      size_t pinNumRight = std::stoi(
          &std::regex_token_iterator(C_ALL(right), afterCol, 0)->str()[1]);
      int gateIndex = findGateIndex(compNameLeft);

      if (compNameRight != "self") {
        int otherGateIndex = findGateIndex(compNameRight);

        this->circuitry[gateIndex]->setLink(
            pinNumLeft, *this->circuitry[otherGateIndex], pinNumRight);
      } else {
  ...
  while (calculated != 2) {
    for (auto &pin : this->pins) {
      if (pin.inner_connection.gate_r != nullptr)
        pin.inner_connection.gate_r->setPin(pin.inner_connection.pin,
                                            *pin.state);
    }
    ...
    for (auto &gate : this->circuitry) {
      ...
      for (pin_t *outPin : gate->outputPins()) {
        if (outPin->outer_connection.pin > 0) {
          *this->pins[this->findPinIndex(outPin->outer_connection.pin)].state =
              *outPin->state;
        } else if (outPin->inner_connection.pin > 0) {
          outPin->inner_connection.gate_r->setPin(outPin->inner_connection.pin,
                                                  *outPin->state);
        }
      }
    }
...
nts::ILogicGate *nts::GenericComponent::fetchGate(const std::string &type,
                                                  const std::string &name) {
  if ("and" == type)
