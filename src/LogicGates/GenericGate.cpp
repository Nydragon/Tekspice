...
std::string nts::GenericGate::getName() { return this->_name; }

void nts::GenericGate::setLink(std::size_t pin, nts::ILogicGate &other,
                               std::size_t otherPin) {
...
void nts::GenericGate::setLink(std::size_t pin, nts::IComponent &other,
                               std::size_t otherPin) {
...
void nts::GenericGate::dump() {
  ...
  std::cout << std::setw(col_w) << "Pin No." << std::setw(col_w) << "outer con"
            << std::setw(col_w) << "outer con Pin" << std::setw(col_w)
            << "inner con" << std::setw(col_w) << "inner con Pin"
            << std::setw(col_w) << "state" << std::endl;
  for (auto &pin : this->pins) {
    std::cout << std::setw(col_w) << pin.number << std::setw(col_w)
              ...
              << (pin.inner_connection.gate_r != nullptr
                      ? pin.inner_connection.gate_r->getName()
                      : "N/A")
              << std::setw(col_w)
              << (pin.inner_connection.gate_r != nullptr
                      ? std::to_string(pin.inner_connection.pin)
                      : "N/A")
              << std::setw(col_w) << TRI(pin.state) << std::endl;
  }
}
